...
void loop() {
  ...
  // if(isBleDisabled){
  //   vTaskDelay(200 / portTICK_PERIOD_MS);  // Sleep for 200 mili second
  // }
  delay(200);
}
  ...
  if (lastState3 == HIGH && currentState3 == LOW) {
    ...
    if (millis() - pressStartTime3 >= 1200) {
      ...
      if(!isBleDisabled){
        ...
        int analogValue = analogRead(ANALOG_PIN);  // Read the analog input
        if(batteryLevelManage(analogValue)){
...
bool batteryLevelManage(int analogValue){
  Serial.println();Serial.print("Analog Input: ");Serial.println(analogValue);
  if(analogValue < batteryLowMargin ){
    return false;
  }
  else{
    return true;
